...
while True:
    ...
    entrada = input("Digite o tamanho do seu amigo em cm (ou 'pare'): ")
    ...
    for i, altura in enumerate(medias):
        ...
        meia_elipse = Wedge(
            (x + largura/2, y + altura - largura/2),
            largura/2,
            0,
            180,
            facecolor=cor_medias,
            edgecolor="black"
        )
    ...
    if altura_usuario <= min_media:
        x_usuario = posicoes_medias[0] - (
            3 * (min_media - altura_usuario) / (max_media - min_media)
        )

    elif altura_usuario >= max_media:
        x_usuario = posicoes_medias[-1] + (
            3 * (altura_usuario - max_media) / (max_media - min_media)
        )
        ...
        for j in range(len(medias)-1):
            if medias[j] <= altura_usuario <= medias[j+1]:
                ...
                delta = (
                    (altura_usuario - medias[j]) /
                    (medias[j+1] - medias[j])
                )

                x_usuario = (
                    posicoes_medias[j] +
                    delta * (posicoes_medias[j+1] - posicoes_medias[j])
                )
    ...
    # desenha usuário
    retangulo_usuario = Rectangle(
        (x_usuario, y_usuario),
        largura,
        altura_usuario - largura/2,
        color=cor_usuario,
        ec="black"
    )
    ...
    meia_elipse_usuario = Wedge(
        (x_usuario + largura/2,
         y_usuario + altura_usuario - largura/2),
        largura/2,
        0,
        180,
        facecolor=cor_usuario,
        edgecolor="black"
    )
    ...
    ax.text(
        x_usuario + largura/2,
        y_usuario + altura_usuario + 0.8,
        f"Você: {altura_usuario} cm",
        ha='center',
        va='bottom',
        fontsize=10
    )
    ...
    # legenda
    ax.text(
        posicoes_medias[-1]+1.5,
        max(max(medias), altura_usuario)/2,
        "Azul: Médias\nVermelho: Você",
        color="black",
        fontsize=12,
    ...
    elif 10 <= altura_usuario <= 17:
        mensagens = [
            "Bem ... é isso ai, continue firme!",
            "Massa, tá de boa!",
            "Você consegue !!",
            "Lembre-se... você ainda é amado",
    ...
    ax.set_ylim(
        -3,
        max(max(medias), altura_usuario)+3
    )

    ax.set_xticks(posicoes_medias)

    ax.set_xticklabels(
        [f"Média {i+1}" for i in range(len(medias))]
    )

    ax.set_ylabel("Tamanho em cm")

    ax.set_title(
        "Comparação das Médias com Você (Proporcional)"
    )
